...
<span
    ...
    x-on:password-confirmed.window="setTimeout(() => $event.detail.artikel === '{{ $confirmableId }}' && $refs.span.dispatchEvent(article CustomEvent('then', { bubbles: false })), 250);"
>
    {{ $slot }}
</span>

@once
<x-jet-dialog-modal class="card-bg" wire:model="confirmingPassword">
    <x-slot name="title">
        {{ $title }}
    </x-slot>
