<div class="p-6 ">
    <h1 class="md:text-2xl pb-10">Article</h1>


    @if (session()->has('message'))

        <div
            class="alerts mb-6 bg-blue-100 border-l-4 border-blue-500 text-blue-700 px-4 py-3"
            role="alert"
        >
    ...
    @if (session('delete'))
        <div
            class="alerts mb-6 bg-orange-100 border-l-4 border-orange-500 text-orange-700 p-4"
            role="alert"
        >
    ...
    <form class="p-6 pb-8 mb-10 pt-4 rounded-xxl card-bg-100 "  wire:submit.prevent="createArticle" >


        <div class="w-full ">
            <label class="py-3 block font-semibold text-lg">Füge Artikel Hinzu:</label>

            <input wire:model.defer="newsInput.title" class="input-filler-color accent-bg textarea w-full  rounded-lg" type="text" name="titel" required placeholder="Titel eingeben.."></input>
            ...
            <textarea wire:model.lazy="newsInput.text" class="accent-bg textarea w-full text-gray-200 rounded-lg" type="text" name="text" required placeholder="Enter Text.."></textarea>


            <div class="flex-col mt-3">
                <label>Read Duration in min:</label>
                <div class="pt-3 ">
                    <input wire:change="$emit('durationChanged')" wire:model="duration" class="input-filler-color accent-bg textarea text-gray-100 rounded-lg w-32 text-center " type="number" name="duration" required placeholder="e.g.: 5 mins "  >
                    @if($duration == 1)
                        min
                    @elseif($duration > 1)
                        mins
        ...
        <div class="flex justify-end">
            <button  class="accent-blue rounded-lg p-2 px-10 text-white font-semibold" type="submit" >Submit</button>
    ...
    <div class="py-10  rounded-xl">
        @foreach($articles as $article)
                ...
                <div class="w-8/12 m-auto flex justify-center flex-col mb-12">
                    <div class="flex justify-end pr-3 pb-3">
                        <a class="flex items-center mr-5" href="{{url('admin/articles/feature',$article->id)}}" >
                            Feature
                            <img class="ml-2" width="27px" src={{url($article->featured_article ? 'svgs/feature-green.svg': 'svgs/feature.svg')}}>
                        </a>

                        <a class="flex items-center" href="{{route('admin.articles.show', $article->id)}}" >
                            Edit
                            <img width="30px" src={{url('svgs/edit.svg')}}>
                        </a>
                    </div>

                    <a href="{{url('articles/our-story/' .$article->id)}}" class="w-full h-auto ">

                        <div class="rounded-img mb-3 h-auto" style="background-image: url('/storage/articles/images/{{$article->image}}');
        ...
        @endforeach
            ...
            {{--                <h1 class="py-2 md:text-base md:font-semibold">{{$article->title}}</h1>--}}

{{--                <a class="flex items-center" href="{{url('admin/articles/feature',$article->id)}}" >--}}
{{--                    <img class="ml-2" width="27px" src={{url($article->featured_article ? 'svgs/feature-green.svg': 'svgs/feature.svg')}}>--}}
{{--                </a>--}}
...
{{--                        <img class=" rounded-t-xl "  src="{{url('storage/articles/images/' . $article->image)}}"/>--}}

{{--                    @endif--}}
{{--                    @if(!empty($article->video))--}}
{{--                        <video class=" rounded-t-xl"  preload="metadata" controls>--}}
...
{{--                    <p class="py-2 md:text-sm md:font-semibold">{{$article->text}}</p>--}}
