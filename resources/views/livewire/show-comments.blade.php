...
<div class="p-2.5 md:p-4 rounded-articles card-bg ">
    ...
    <div x-data="{ showReplies: true}"  class=" comment-border"  >
            <div x-data="{ showReplyWindow: false}">
                ...
                <div  class="pt-4 mb-10 flex items-start w-full pb-3">
                    <div class="flex pr-2">
                    ...
                    <div  class="flex-1 flex justify-between rounded-xl p-2 px-3  accent-bg ">
                        <div>
                            ...
                            <h2 class="font-medium">{{$comment->user->name}}:</h2>

                            <p class="self-end first-word">{{$comment->comment}}</p>
                        ...
                        <div   class="self-end " >
                            ...
                            @isset($comment->commentLikes)
                                ...
                                @endforeach
                                <li class="cursor-pointer flex items-center" wire:click="likeComment({{$comment->id}}, {{$commentLikeCount}})">
                                    <div class="flex">
                                        Likes:
...
<br>
                <div class="border-t border-gray-400 mb-10"></div>
            </div>

            <div x-show="showReplies"
                 x-transition.opacity.duration.400ms class="mt-4">
{{--                {{$comment->with('replies')->where('parent_id', $comment->id)->get()}}--}}

{{--                @for()--}}
{{--                {{Post::with('comments.replies')->get()}}--}}
                @foreach($comment->with('replies')->where('comment_id', $comment->id)->get() as $replies)
                    @once
